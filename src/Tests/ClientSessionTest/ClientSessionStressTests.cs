...
public class ClientSessionStressTests
{
    ...
    public async Task StressTest_NetworkChaos()
    {
        // Arrange
        ...
        const int maxMessageSize = 16384; // 16 KB
        ...
        Task receiverTask = Task.Run(async () =>
        {
            byte[] buffer = new byte[maxMessageSize * 2];
            StringBuilder accumulated = new StringBuilder();
            ...
            while (!receiverCts.Token.IsCancellationRequested)
            {
                try
                {
                    ...
                    // Process delimited messages
                    string current = accumulated.ToString();
                    int lastPos = 0;
                    int pos;
                        
                    while ((pos = current.IndexOf(Delimiter, lastPos)) != -1)
                    {
                        Interlocked.Increment(ref receivedMessages);
                        lastPos = pos + 1;
                    }
        ...
        while (DateTime.UtcNow < endTime)
        {
            try
            {
                ...
                if (operation < 7) // 70% normal sends
                {
                    // Send a normal message (variable size)
                    int size = random.Next(10, maxMessageSize);
                    string message = new string('X', size - 1) + Delimiter;
                    await session.SendAsync(message);
                    Interlocked.Increment(ref sentMessages);
                }
                else if (operation == 7) // 10% very large messages
                {
                    string largeMessage = new string('X', maxMessageSize - 1) + Delimiter;
                    await session.SendAsync(largeMessage);
                    Interlocked.Increment(ref sentMessages);
                }
                else if (operation == 8) // 10% reconnection
                {
                    // Force reconnection
                    await session.StopAsync();
                    clientSocket.Close();
                    ...
                    receiverCts.Cancel();
                    await Task.WhenAny(receiverTask, Task.Delay(500));
                        
                    var newPair = await CreateClientSessionPairAsync();
                    session = newPair.session;
                    clientSocket = newPair.clientSocket;
                    sessionTask = newPair.sessionTask;
                ...
                else if (operation == 9) // 10% rapid burst
                {
                    ...
                    for (int i = 0; i < burstSize; i++)
                    {
                        ...
                        string message = new string('X', size - 1) + Delimiter;
                        burstTasks[i] = session.SendAsync(message);
                        Interlocked.Increment(ref sentMessages);
                    }
            ...
            {
                Console.WriteLine($"Chaos operation error: {ex.Message}");
                Interlocked.Increment(ref errors);
                    
                // Brief delay after errors
                await Task.Delay(50);
            }
        }
        ...
        Console.WriteLine($"Messages sent: {sentMessages}");
        Console.WriteLine($"Messages received: {receivedMessages}");
        Console.WriteLine($"Reconnections: {reconnections}");
        Console.WriteLine($"Errors encountered: {errors}");
        Console.WriteLine($"Message delivery rate: {(double)receivedMessages / sentMessages:P2}");
            
        // Even with chaos, we should have decent message delivery
        Assert.IsTrue(receivedMessages > 0, "Should receive at least some messages");
